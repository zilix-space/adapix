...
export default function TransactionCompleted({
  ...
  return (
    <Html>
      <Head />
      <Preview>Transação Concluída - {APP_CONFIGS.app.name}</Preview>
      <Tailwind>
        <Body className="mx-auto my-auto bg-white font-sans">
          <Container className="mx-auto my-10 max-w-[500px] rounded-md border border-solid border-gray-200 px-10 py-5">
            ...
            <Heading className="mx-0 my-7 p-0 text-left text-xl font-semibold text-black">
              Transação Concluída - {APP_CONFIGS.app.name}
            </Heading>
            <Text className="text-sm leading-6 text-black">
              Olá{name && `, ${name}`}!
            </Text>
            <Text className="text-sm leading-6 text-black">
              Sua transação foi concluída com sucesso. Aqui estão os detalhes da
              transação:
            </Text>
            <Text className="text-sm leading-6 text-black">
