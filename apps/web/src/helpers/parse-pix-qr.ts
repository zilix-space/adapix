...
export function parsePixQR(qrString: string): PixQRData | null {
  ...
  try {
    ...
    const extract = (id: string, data: string): string | null => {
      try {
        ...
        if (match && match.length >= 3) {
          ...
          console.log(`Extraído [${id}]: comprimento=${length}, valor="${extracted}"`);
          return extracted;
        }
    ...
    if (Object.keys(pixData).length <= 1 && qrString.includes('BR.GOV.BCB.PIX')) {
    ...
    if (Object.keys(pixData).length > 1) { // > 1 porque raw está sempre presente
      return pixData;
    }
